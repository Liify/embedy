...
static void TIM6_Init(void) {
    ...
    // force update of the parameters
    TIM6->EGR |= (1U << 0);
...
static void TIM6_Delay_ms() {
    // wait until the flag goes off ( do that beforehand so we can catch up if something delays)
    while ((TIM6->SR & (1U << 0)) == 0) {
    }

    TIM6->SR &= ~(1U << 0); // reset the flag
}
