#ifndef GPIO_H
#define GPIO_H

#include <stdint.h>

// peryferia strona 77
#define RCC_BASE 0x40021000U
#define GPIOC ((GPIO_TypeDef *) 0x48000800U)

// power registers strona 251
#define RCC_AHB2ENR (*((volatile uint32_t *)(RCC_BASE + 0x4CU)))
#define RCC_AHB2ENR_GPIOCEN (1U << 2) // 251

// gpio strona 311
typedef struct {
    volatile uint32_t MODER;
    volatile uint32_t OTYPER;
    volatile uint32_t OSPEEDR;
    volatile uint32_t PUPDR;
    volatile uint32_t IDR;
    volatile uint32_t ODR;
    volatile uint32_t BSRR;
    volatile uint32_t LCKR;
    volatile uint32_t AFRL;
    volatile uint32_t AFRH;
    volatile uint32_t BRR;
    volatile uint32_t ASCR;
} GPIO_TypeDef;

typedef enum {
    GPIO_MODE_INPUT = 0x0U, // 00
    GPIO_MODE_OUTPUT = 0x1U, // 01
    GPIO_MODE_ALTERNATE = 0x2U, // 10
    GPIO_MODE_ANALOG = 0x3U // 11
} GPIO_Mode;

void GPIO_Init(GPIO_TypeDef *port, uint8_t pin, GPIO_Mode mode);

void GPIO_SetAlternateFunction(GPIO_TypeDef *port, uint8_t pin, uint8_t af_num);

#endif
